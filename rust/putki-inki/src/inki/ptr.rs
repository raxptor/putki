    ...
    fn resolve_notrack(&self) -> Option<Arc<T>> {
        match self.target {
            ...
            PtrTarget::ObjPath {
                ...
                if let source::ResolveStatus::Resolved(ptr) =
                    source::resolve_from::<T>(resolver, path)
                {
            ...
            PtrTarget::InlineObject {
                ref data,
                ref resolver,
                ref type_name,
                ..
            } => Some(Arc::new(<T as source::ParseFromKV>::parse_with_type(
                data, resolver, type_name,
            ))),
...
impl<Target> fmt::Debug for Ptr<Target>
where
    Target: source::ParseFromKV,
{
    fn fmt(&self, f: &mut fmt::Formatter) -> result::Result<(), fmt::Error> {
        match self.target {
            PtrTarget::Null => {
                write!(f, "Null pointer").ok();
            }
            PtrTarget::ObjPath { ref path, .. } => {
                write!(f, "Ptr path[{}]", &path).ok();
            }
            PtrTarget::InlineObject {
                ref type_name,
                ref path,
                ...
                write!(f, "Ptr inline object type[{}] path={}", type_name, path).ok();
            }
            PtrTarget::TempObject { ref path, .. } => {
                write!(f, "Temp object path={}", path).ok();
            }
        }
        Ok(())
    }
...
impl<Target> source::WriteAsText for Ptr<Target>
where
    Target: source::WriteAsText + shared::TypeDescriptor,
    Self: PtrInkiResolver<Target>,
{
...
impl<Target> Ptr<Target> {
    ...
    pub fn new_temp_object(path: &str, obj: Arc<Target>) -> Ptr<Target>
    where
        Target: source::ParseFromKV,
    {
        Ptr {
...
pub fn ptr_from_data<T>(resolver: &Arc<source::InkiResolver>, ld: &lexer::LexedData) -> Ptr<T>
where
    T: source::ParseFromKV,
{
    match *ld {
