...
impl BinWriter for f32 {
    fn write(&self, data: &mut Vec<u8>) {
        u32::write(&f32::to_bits(*self), data);
    }
}
    ...
    T: Send + Sync,
{
    fn write(&self, data: &mut Vec<u8>, refs: &PackageRefs) -> Result<(), PutkiError> {
        let slot: i32 = self
            .get_target_path()
            .and_then(|x| refs.path_to_slot.get(x))
            .map(|x| (*x) as i32)
            .unwrap_or(-1);
        slot.write(data);
        Ok(())
...
pub fn insert_value<T>(data: &mut Vec<u8>, offset: usize, value: T)
where
    T: BinWriter,
{
    let mut tmp = Vec::new();
