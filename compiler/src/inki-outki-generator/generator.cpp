...
namespace putki
{
	...
	void write_runtime_header(putki::parsed_file *file, runtime::descptr rt, putki::indentedwriter out)
	{
		...
		for (size_t i=0; i!=file->structs.size(); i++)
		{
			...
			if (rt)
			{
				out.line() << "#pragma pack(push, " << rt->struct_align << ")";
			}
			
			out.line() << "struct " << to_c_struct_name(s->name) << " {";
			out.indent(1);
			...
			for (size_t i=0; i<s->fields.size(); i++)
			{
					...
					switch (s->fields[i].type)
					{
						...
						case FIELDTYPE_POINTER:
							if (rt)
								...
								out.cont() << "outki::" << to_c_struct_name(f->ref_type) << " *";
							break;
						case FIELDTYPE_ENUM:
							if (rt)
								out.cont() << rt_wrap_field_type(FIELDTYPE_ENUM, rt) << " ";
							else
								out.cont() << "outki::" << to_c_enum_name(f->ref_type) << " ";
							break;
				...
				out.cont() << to_c_field_name(f->name) << ";";

				if (s->fields[i].is_array)
					out.line() << "unsigned int " << to_c_field_name(f->name) << "_size;";
			}
			...
			out.line();
			out.line() << "// loading processing for " << s_name;
			out.line() << "char* post_blob_load_" << s_name << "(" << s_name << " *d, char *beg, char *end);";
			...
			if (rt)
			{
				out.line() << "char *write_" << s_name << "_aux(inki::" << s_name << " *in, outki::" << s_name << " *d, char *out_beg, char *out_end);";
			}
		}
	...
	void write_runtime_impl(putki::parsed_file *file, runtime::descptr rt, putki::indentedwriter out)
	{
		...
		for (size_t i=0; i!=file->structs.size(); i++)
		{
			...
			out.line() << "char* post_blob_load_" << s_name << "(" << s_name << " *d, char *aux_cur, char *aux_end)";
			out.line() << "{";
			out.indent(1);

			if (s->is_type_root)
				out.line() << "d->rtti_type_ref() = " << s_name << "::type_id();";
			...
			for (size_t j=0; j<s->fields.size(); j++)
			{
				...
				std::string fname = to_c_field_name(s->fields[j].name);
				std::string fref = std::string("d->") + fname;

				if (s->fields[j].is_array)
				{
					if (s->fields[j].type == FIELDTYPE_STRUCT_INSTANCE)
					{
						out.line() << "d->" << fname << " = reinterpret_cast<outki::" << s->fields[j].ref_type << "*>(aux_cur);";
						out.line() << "aux_cur += sizeof(" << s->fields[j].ref_type << ") * d->" << fname << "_size;";
					}
					else if (s->fields[j].type == FIELDTYPE_POINTER)
					{
						out.line() << "d->" << fname  << " = reinterpret_cast<" << s->fields[j].ref_type << "**>(aux_cur);";
						out.line() << "aux_cur += sizeof(" << s->fields[j].ref_type << "*) * d->" << fname << "_size;";
					}
					else
					{
						out.line() << "d->" << fname << " = reinterpret_cast<" << win32_field_type(s->fields[j].type) << "*>(aux_cur);";
						out.line() << "aux_cur += sizeof(" << win32_field_type(s->fields[j].type) << ") * d->" << fname << "_size;";
					}

					if (needs_loop)
					{
						out.line() << "if (aux_cur > aux_end) return 0; ";
						out.line() << "for (unsigned int i=0;i<d->" << fname + "_size;i++)";
						out.line() << "{";
						out.indent(1);
					}
					fref = fref + "[i]";
	...
	void write_putki_header(putki::parsed_file *file, putki::indentedwriter out)
	{
		...
		for (size_t i=0; i!=file->structs.size(); i++)
		{
			...
			out.line();
			out.line() << "putki::type_handler_i *get_" << s_name << "_type_handler();";
			out.line() << "putki::ext_type_handler_i *get_" << s_name << "_ext_type_handler();";
			out.line() << "void fill_" << s_name << "_from_parsed(putki::parse::node *pn, void *target_, putki::load_resolver_i *resolver);";
			
			out.line();
			out.line() << "struct " << s_name << " {";
			out.indent(1);

			out.line();
			out.line() << "static inline " << s_name << " * alloc() { return (" << s_name << " *) get_" << s_name << "_type_handler()->alloc(); }";
			out.line() << "static inline putki::type_handler_i * th() { return get_" << s_name << "_type_handler(); }";
			...
			for (size_t j=0; j<s->fields.size(); j++)
			{
				...
				{
					out.line() << f_name << " = " << s->fields[j].def_value << ";";
				}
				else
				{
					switch (s->fields[j].type)
			...
			{
				std::string f_name = to_c_field_name(s->fields[j].name);
				std::string cont = f_name + ";";

				if (s->fields[j].is_build_config)
				{
					cont = "& " + f_name + "(const char *build_config) {";
				}
			...
			{
				out.line();
				out.line() << "// writing processing for " << s_name;
	...
	{
		std::string f_name = to_c_field_name(f->name);
		std::string ref = "target->" + f_name;
		...
		if (f->is_array)
		{
			...
			out.line() << "  " << type << " tmp;";
			out.line() << "  (void)n;";
			out.line() << "  target->" << f_name << ".push_back(tmp); i++;";
			out.indent(-1);
			out.line() << "}";
			out.line() << "i = 0;";
			out.line() << "while (putki::parse::node * n = putki::parse::get_array_item(arr, i)) {";
			out.indent(1);
			out.line() << type << " & fixed_mem_obj = target->" << f_name << "[i];";
		...
		{
			out.line() << "fill_" << to_c_struct_name(f->ref_type) << "_from_parsed(" << node << ", &" << ref << ", resolver);";
		}
	...
	void write_putki_parse(putki::parsed_file *file, putki::indentedwriter out)
	{
		...
		for (int i=0; i!=file->structs.size(); i++)
		{
			...
			out.line() << "{";
			out.indent(1);
			out.line() << s_name << " *target = (" << s_name << " *) target_;";
	...
	void write_ptr_walker(putki::parsed_file *file, putki::indentedwriter out, bool runtime)
	{
		...
		for (int i=0; i!=file->structs.size(); i++)
		{
			...
			for (size_t j=0; j<s->fields.size(); j++)
			{
				...
				std::string ref = "input->" + f_name;

				if (fd.is_array)
				{
					if (!runtime)
					{
						out.line() << "for (unsigned int i=0;i<input->" << f_name << ".size();i++) {";
						ref = "input->" + f_name + "[i]";
					}
					else
					{
						out.line() << "for (unsigned int i=0;i<input->" << f_name << "_size;i++) {";
						ref = "input->" + f_name + "[i]";
					}
					out.indent(1);
				}
				...
				if (fd.type == putki::FIELDTYPE_STRUCT_INSTANCE)
				{
					...
					out.line() << "walk_dependencies_" << to_c_struct_name(fd.ref_type) << "(&" << ref << ", walker";
	...
	void write_json_writer(putki::parsed_struct *s, putki::indentedwriter out)
	{
		...
		for (size_t j=0; j<copy.size(); j++)
		{
			...
			else if (fd.type == FIELDTYPE_STRUCT_INSTANCE)
			{
				...
				out.line() << "{";
				out.line(1) << "putki::type_handler_i *thandler = inki::get_" << to_c_struct_name(fd.ref_type) << "_type_handler();";
	...
	void write_putki_type_reg(putki::parsed_file *file, putki::indentedwriter out)
	{
		...
		for (int i=0; i!=file->structs.size(); i++)
		{
			...
			out.line();
			out.line() << "// type handler for " << s_name << " (" << s->name << ")";
			out.line() << "// parent=[" << s->parent << "] is_type_root=" << s->is_type_root;;

			out.line() << "struct " << s_name << "_handler : public putki::type_handler_i {";
			out.indent(1);			
			out.line();

			if ((!s->parent.empty()) || s->is_type_root)
				out.line() << "putki::instance_t alloc() { " << s_name << " *tmp = new " << s_name << "; tmp->rtti_type_ref() = " << s->unique_id << "; return tmp; }";
			else
				out.line() << "putki::instance_t alloc() { return new " << s_name << "; }";

			out.line() << "putki::instance_t clone(putki::instance_t source) { " << s_name << " *tmp = (" << s_name << "*) alloc(); *tmp = *((" << s_name << " *)source); return tmp; }";
			out.line() << "void free(putki::instance_t p) { delete (" << s_name << "*) p; }";
			out.line();
			out.line() << "// info";
			...
			if (!s->parent.empty())
				out.cont() << "get_" << to_c_struct_name(s->parent) << "_type_handler();";
			else
				out.cont() << "0;";
			...
			if (s->domains & putki::DOMAIN_RUNTIME)
			{
				for (int p=0;; p++)
				{
					...
					if (!t) break;
					write_blob_writer_call(t, to_c_struct_name(s->name).c_str(), out);
				}
			}
			...
			out.line(1) << "fill_" << s_name << "_from_parsed(pn, target, resolver);";
			out.line() << "}";
			out.line();
			out.indent(-1);
			out.line() << "} s_" << s_name << "_handler;";
			out.line();
			out.line() << "void bind_type_" << s_name << "() { putki::typereg_register(\"" << s->name << "\", &s_" << s_name << "_handler); } ";
			out.line() << "putki::type_handler_i *get_" << s_name << "_type_handler() { return &s_" << s_name << "_handler; }";
		}
	...
	void write_blob_writers(putki::parsed_file *file, putki::indentedwriter out, runtime::descptr rt)
	{
		...
		for (size_t i=0; i!=file->structs.size(); i++)
		{
			...
			out.line();
			out.line() << "char *write_" << s_name << "_aux(inki::" << s_name << " *in, " << out_ns << s_name << " *d, char *out_beg, char *out_end)";
			out.line() << "{";
			out.indent(1);
			...
			for (size_t j=0; j<s->fields.size(); j++)
			{
				...
				std::string srcd = "in->" + f_name;
				std::string outd = "d->" + f_name;
				...
				if (fd.type == FIELDTYPE_STRING || fd.type == FIELDTYPE_PATH)
					...
					out.line() << "out_beg = write_" << to_c_struct_name(fd.ref_type) << "_aux(&" << srcd << ", &" << outd << ", out_beg, out_end);";
				else if (fd.type == FIELDTYPE_POINTER)
			...
			std::string out_n(out_ns + s_name);
			...
			out.line() << "return write_" << s_name << "_aux(in, d, out_beg + sizeof(" << out_n << "),  out_end);";
			out.indent(-1);
			out.line() << "}";
	...
	void write_putki_impl(putki::parsed_file *file, putki::indentedwriter out)
	{
		...
		out.line();
		write_comment_block("Parsers", out);
		out.line();
		write_putki_parse(file, out);
		out.line();
		write_comment_block("Ptr walkers", out);
		out.line();
	...
	void write_bind_decl(putki::parsed_file *file, putki::indentedwriter out)
	{
		...
		{
			putki::parsed_struct *s = &file->structs[i];
			out.line() << "void bind_type_" << to_c_struct_name(s->name) << "();";
		}
		out.line() << "}";
	}
	...
	void write_runtime_blob_load_cases(putki::parsed_file *file, putki::indentedwriter out)
	{
		for (unsigned int i=0; i<file->structs.size(); i++)
		{
			...
			out.line() << "{";
			out.line(1) << "char *out = post_blob_load_" << s_name << "((" << s_name << "*)begin, begin + sizeof(" << s_name << "), end);";
			out.line(1) << "if (out) walk_dependencies_" << s_name << "((" << s_name << "*)begin, ptr_reg);";
