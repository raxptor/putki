...
public class Compiler {
	...
	TypeMap[] stdTypes = new TypeMap[] { new TypeMap("u32", FieldType.UINT32), new TypeMap("uint", FieldType.UINT32),
			new TypeMap("s32", FieldType.INT32), new TypeMap("int", FieldType.INT32),
			new TypeMap("bool", FieldType.BOOL), new TypeMap("byte", FieldType.BYTE),
			new TypeMap("string", FieldType.STRING), new TypeMap("hash", FieldType.HASH),
			new TypeMap("float", FieldType.FLOAT), new TypeMap("file", FieldType.FILE),
	...
	public boolean parseStructLine(ParsedStruct cur, String line) {
		...
		for (int k = 0; k < pieces.length; k++) {
			if (pieces[k].equals("=")) {
				...
				if (pieces[k].length() > 2 && pieces[k].charAt(0) == '{'
						&& pieces[k].charAt(pieces[k].length() - 1) == '}') {
					next.localizationCategory = pieces[k].substring(1, pieces[k].length() - 1);
					continue;
				} else if (pieces[k].length() > 3 && pieces[k].charAt(0) == '{'
						&& pieces[k].charAt(pieces[k].length() - 2) == '}'
						&& pieces[k].charAt(pieces[k].length() - 1) == '+') {
				...
				System.out
						.println("Unexpected token when parsing field. Line [" + line + "] piece=[" + pieces[k] + "]");
				return false;
			}
		}
		...
		if (gotType && !readRefType) {
			...
			System.out.println("line [" + line + "] parse status gotType:" + gotType + " type=" + next.type + "  name:"
					+ next.name + " readRef:" + readRefType + " readDef:" + readDefValue);
		}
		return false;
	}
