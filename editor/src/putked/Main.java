...
public class Main extends Application
{
	...
	public static DataLoader s_dataLoader;
	public static DataWriter s_dataWriter;
	public static Compiler s_compiler;
    ...
    public void startEditing(String path, Editor editor)
    {
    	...
    	if (obj == null)
    		return;
    	if (!editor.canEdit(obj.getType()))
    		return;

    	for (int i=0;i<m_openEditors.size();i++)
    	{
    		OpenEditors oe = m_openEditors.get(i);
    		if (oe._mi.getPath().equals(obj.getPath())) // && oe._editor == editor)
    		{
    	...
    	Tab t = addTab(obj, editor.createUI(obj), path);
    	...
    	oe._mi = obj;
    	oe._tab = t;
    	oe._contentHash = obj.getContentHash();
...
//    	s.getIcons().add(new Image(getClass().getResourceAsStream("misc/putkico-5.png")));
    ...
    public String askForInstancePath(Compiler.ParsedStruct p)
    {
    ...
    public Compiler.ParsedStruct askForType()
    {
    	return askForSubType(null, false);
    }

    public Compiler.ParsedStruct askForSubType(Compiler.ParsedStruct p, boolean asAux)
    {
    	List<Compiler.ParsedStruct> all = s_compiler.getAllTypes();
    	ObservableList<TypeOption> out = FXCollections.observableArrayList();
    	for (Compiler.ParsedStruct t : all)
    	{
    		if (p == null || t.hasParent(p) && (!asAux || t.permitAsAux))
    		{
    			TypeOption to = new TypeOption();
    			to.name = "[" + t.moduleName + "] - " + t.name;
    			to.type = t;
    			out.add(to);
    		}
    	}
    	...
    	class Tmp {
    		Compiler.ParsedStruct out = null;
    	};
    ...
    public void start(Stage stage) throws BackingStoreException, IOException
    {
    	...
    	s_dataWriter = new DataWriter(m_objsPath);
		...
		if (!s_compiler.compile(m_projectPath))
		{
			System.out.println("Could not compile typedefs @ " + m_projectPath + "! Aborting!");
			return;
		}
