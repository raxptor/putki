...
# tests/simple's typedefs cover inheritance, polymorphism, arrays, enums, nested
# structs and build configs, so this catches generator regressions that produce
# code which does not build -- which unit tests over hand-written vectors miss.
